...
class CNN(nn.Module):
    def __init__(self, sequence_length):
        ...
        self.conv1 = nn.Conv1d(1, 20, 7, 1, 3)
        self.conv2 = nn.Conv1d(20, 20, 5, 1, 2)
        self.fc1 = nn.Linear(20 * sequence_length, 100)
        self.fc2 = nn.Linear(100, sequence_length)

    def forward(self, x):
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = x.view(-1, 20 * self.sequence_length)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        x = x.view(-1, 1, self.sequence_length)
...
class CNN_MODULE(Disaggregator):
    ...
    def partial_fit(self, train_mains, train_appliances, pretrained=False, do_preprocessing=True):
        ...
        for app_name, app_power in train_appliances:
            ...
            if not os.path.exists("./run/"+app_name+"_CNN_pruned_quantized_best_state_dict.pth"):
                train(app_name, model, train_mains, app_power, self.n_epochs, self.batch_size, self.train_patience)
            else:
                self.models[app_name].load_state_dict(torch.load("./run/"+app_name+"_CNN_pruned_quantized_best_state_dict.pth"))
    ...
    def set_appliance_params(self, train_appliances):
        for (app_name, app_df_lst) in train_appliances:
            ...
            print("appliance:{} mean:{} std:{}".format(app_name, app_mean, app_std))
...
def train(appliance_name, model, train_mains, appliance_mains, epochs, batch_size, train_patience, checkpoint_interval=None, pretrain=False):
    ...
    for epoch in range(epochs):
        ...
        if best_loss is None or avg_loss < best_loss:
            ...
            path_state_dict = "./"+appliance_name+"_CNN_pruned_best_state_dict.pth"
