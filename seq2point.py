...
class seq2point(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x).view(-1, 50 * self.sequence_length)
        x = self.dense(x)
        return x.view(-1, 1)
...
# test the input and output of the seq2point module
'''
sequence_length = 256
model = seq2point(sequence_length=sequence_length)
input = torch.ones((256, 1, sequence_length))
print(input.shape)
output = model(input)
print(output.shape)
'''
...
class Seq2Point(Disaggregator):
    ...
    def partial_fit(self, train_mains, train_appliances, pretrained=False, do_preprocessing=True):
        ...
        for app_name, app_power in train_appliances:
            if app_name not in self.models:
                ...
                self.models[app_name] = seq2point(self.sequence_length)

            model = self.models[app_name]
            train(app_name, model, train_mains, app_power, self.n_epochs, self.batch_size, self.train_patience)
            self.models[app_name].load_state_dict(torch.load("./"+app_name+"_seq2point_best_state_dict.pt"))
        ...
        if method == 'train':
            ...
            for (app_name, app_df_lst) in submeters_lst:
                ...
                for app_df in app_df_lst:
                    # Different from Seq2Seq model, no need for padding, only normalization
                    appliance = app_df.values.reshape((-1, 1))
                    appliance = (appliance - app_mean) / app_std
    ...
    def normalize_data(self, data, sequence_length, mean, std, overlapping):
        n = sequence_length
        units_to_pad = n // 2
        arr = np.pad(data.flatten(), (units_to_pad, units_to_pad), 'constant', constant_values=(0, 0))
...
def train(appliance_name, model, train_mains, appliance_mains, epochs, batch_size, train_patience, checkpoint_interval=None, pretrain=False):
    ...
    print(train_mains.shape, '\n', appliance_mains.shape)
    ...
    for epoch in range(epochs):
        ...
        if (checkpoint_interval != None) and ((epoch+1) % checkpoint_interval == 0):
            checkpoint = {"model_state_dict": model.state_dict(),
                        ...
                        "epoch": epoch}
            path_checkpoint = "./"+appliance_name+"_seq2point_checkpoint_epoch{}.pt".format(epoch+1)
...
if __name__ =='__main__':
    ...
    'methods':{"Seq2Point":Seq2Point({'n_epochs':5,'batch_size':256, 'sequence_length':129})},
